...
const Schools = () => {
  ...
  const handleOnClickDelete = (data) => {
    if (data.id) {
      api.delete(`api/schoolBranches/delete?id=${data?.id}`).then(response => {
        const filterSchools = schools.filter(school => school.id !== data.id);
  ...
  const fetchSchools = (page, pageSize) => {
    api.post(`api/schoolBranches/getAll/${userDetails.getAccountId()}`, {
      page: page,
      size: pageSize,
      sortBy: "id",
      sortDir: "asc",
