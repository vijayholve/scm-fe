...
const EditTimetable = ({ ...others }) => {
  ...
  useEffect(() => {
    ...
    if (loggedInUserData?.accountId) {
      ...
      fetchData(0, 100, 'api/users/getAll', setTeachers, 'TEACHER');
    }
   ...
   */
  const fetchData = async (page, pageSize, endpoint, setter, typeFilter = '') => {
    try {
      const accountId = loggedInUserData?.accountId || 0; // Use accountId from loggedInUserData
      let url = `${endpoint}/${accountId}`; // Base URL

      // Append type filter if provided and applicable
      if (typeFilter) {
        // If api/teachers/getAll should actually be api/users/getAll with a type filter
        if (endpoint === 'api/teachers/getAll') {
          url = `api/users/getAll/${accountId}?type=${typeFilter}`;
        } else {
          url += `?type=${typeFilter}`; // For other endpoints that might use a type filter
        }
      }
