...
const StudentQuizList = () => {
    ...
    console.log("student quiz list", user?.user?.data);
    useEffect(() => {
    ...
    const fetchAvailableQuizzes = async () => {
        try {
            setLoading(true);
            const response = await api.post(`/api/quizzes/getAll/${user?.user?.data?.accountId}/${user?.user?.data?.schoolId}/${user?.user?.data?.classId}`, {
                page: 0,
                size: 10,
                sortBy: "id",
                sortDir: "asc"
    ...
    const handleStartQuiz = async (quizId) => {
        try {
            ...
            const response = await api.post(`api/quiz/student/start/${quizId}`, {
            ...
            if (response.data.success) {
                ...
                toast.error(response.data.message || 'Cannot start quiz at this time');
            }
        } catch (err) {
            console.error(err);
