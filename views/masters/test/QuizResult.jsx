...
const QuizResult = () => {
    ...
    const fetchQuizResult = async () => {
        try {
            setLoading(true);
            const response = await api.get(`api/quiz/student/result/${quizId}?studentId=${userDetails.getUserId()}`);
    ...
    return (
        <MainCard title="Quiz Result">
            <Grid container spacing={gridSpacing}>
                ...
                {/* Question Review */}
                {result.quiz.showScoreAfterSubmission && result.questionResults && (
                    <Grid item xs={12}>
                        <Card>
                            <CardContent>
                                    ...
                                    <Accordion key={index} sx={{ mt: 1 }}>
                                        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                                            <Box display="flex" alignItems="center" gap={2} width="100%">
                                                ...
                                                <Chip 
                                                    icon={questionResult.isCorrect ? <CheckCircleIcon /> : <CancelIcon />}
                                                ...
                                                <Box display="flex" flexDirection="column" gap={1}>
                                                    <Typography variant="body2">
                                                        <strong>Your Answer:</strong> {
                                                            Array.isArray(questionResult.studentAnswer) 
                                                                ? questionResult.studentAnswer.join(', ') 
                                                                : questionResult.studentAnswer || 'Not answered'
                                                        }
                                                    </Typography>
